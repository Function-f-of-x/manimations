...
class FunctionLogo(MovingCameraScene):
    def construct(self):
        ...
        self.play(Create(function), run_time=8/3, rate_func=lambda t: rate_function(lambda u: (np.sin(3*(-8 + 16*u)+42) 
            + np.sin(np.pi*(3*(-8 + 16*u)+42)) 
            + np.sin(np.e*(3*(-8 + 16*u)+42)) 
            + np.sin(np.sqrt(2)*(3*(-8 + 16*u)+42)))/2 + 2.01, t))
        self.play(Create(label))
        self.play(Create(text))
        self.play(Create(border))
        self.wait()
